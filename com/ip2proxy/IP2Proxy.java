...
public class IP2Proxy {
	...
	private enum Modes {
		COUNTRY_SHORT, COUNTRY_LONG, REGION, CITY, ISP, PROXY_TYPE, IS_PROXY, ALL;
	}
	 ...
	 * @return -1 if error, 0 if not a proxy, 1 if proxy except DCH, 2 if proxy and
	 *         DCH
	 */
	...
	private boolean LoadBIN() throws IOException {
		...
		try {
			if (_IPDatabasePath.length() > 0) {
				...
				_IPv6ColumnSize = 16 + ((_DBColumn - 1) << 2); // 4 bytes each column, except IPFrom column which is 16
																// bytes
				...
				PROXYTYPE_POSITION_OFFSET = (PROXYTYPE_POSITION[_DBType] != 0) ? (PROXYTYPE_POSITION[_DBType] - 1) << 2
						: 0;
				...
				final MappedByteBuffer _IndexBuffer = InChannel.map(FileChannel.MapMode.READ_ONLY, _IndexBaseAddr - 1,
						_BaseAddr - _IndexBaseAddr); // reading indexes
		...
		try {
			...
			while (Low <= High) {
				...
				if (IPNo.compareTo(IPFrom) >= 0 && IPNo.compareTo(IPTo) < 0) {
					...
					if (IPType == 6) { // IPv6
						RowOffset = RowOffset + 12; // coz below is assuming all columns are 4 bytes, so got 12 left to
													// go to make 16 bytes total
					}

					if (PROXYTYPE_ENABLED) {
						if (Mode == Modes.ALL || Mode == Modes.PROXY_TYPE || Mode == Modes.IS_PROXY) {
							Proxy_Type = ReadStr(Read32(RowOffset + PROXYTYPE_POSITION_OFFSET, Buf, RF).longValue(),
									RF);
						}
					}

					if (COUNTRY_ENABLED) {
						if (Mode == Modes.ALL || Mode == Modes.COUNTRY_SHORT || Mode == Modes.COUNTRY_LONG
								|| Mode == Modes.IS_PROXY) {
			...
			if (IP2.equals("::")) {
				...
				if (Mat.matches()) {
					...
					if (Mat2.matches()) {
						...
						long[] Bytes = { 0, 0, 0, 0 }; // using long in place of bytes due to 2's complement signed
														// issue
	...
	private BigInteger Read32Or128(final long Position, final int IPType, final MappedByteBuffer Buf,
			final RandomAccessFile RH) throws IOException {
		if (IPType == 4) {
	...
	private BigInteger Read128(final long Position, final MappedByteBuffer Buf, final RandomAccessFile RH)
			throws IOException {
	...
	private BigInteger Read32(final long Position, final MappedByteBuffer Buf, final RandomAccessFile RH)
			throws IOException {
		if (_UseMemoryMappedFile) {
			// simulate unsigned int by using long
			return BigInteger.valueOf(Buf.getInt((int) Position) & 0xffffffffL); // use absolute offset to be
																					// thread-safe
		} else {
			final int BSize = 4;
			RH.seek(Position - 1);
		...
		if (_UseMemoryMappedFile) {
			...
			try {
				...
				for (int x = 0; x < Size; x++) {
					CBuf[x] = (char) _MapDataBuffer.get((int) Position + 1 + x); // use absolute offset to be
																					// thread-safe
				}
